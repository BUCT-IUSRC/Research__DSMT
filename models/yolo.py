...
#                 y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i].to(x[i].device)  # wh 大小放缩公式见issue
...
class Model(nn.Module):  # 核心模型
    def __init__(self, cfg='yolov5s.yaml', ch=3, ch2=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        self.model, self.save, self.depth_1 = parse_model(deepcopy(self.yaml), ch=[ch], ch2 = [ch2])     #ch2 = 3 深度图像输入通道数  # model, savelist 解析配置文件
        self.save.append(22)  # 增加记录分类层 cafuse 21 fuse_2 20 fuse_3 28 fuse_4 22 sa 22
        self.save.append(37)  # 增加记录分割层 cafuse 36 fuse_2 35 fuse_3 43 fuse_4 37 sa 40
        ...
        if isinstance(m, Detect):
            ...
            m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(2, ch, s, s), torch.zeros(2, ch2, s, s))[0]])  # forward
    ...
    def forward(self, x2, x1, augment=False, profile=False):
        if augment:
            ...
            return self.forward_once(x2, x1, profile)  # single-scale inference, train

    def forward_once(self, x2, x1, profile=False):     #RGB Depth
        y, dt = [], []  # outputs
        for m in self.model:
            if m.i < self.depth_1:
                if m.f != -1:  # if not from previous layer 非单纯上一层则需要调整此层输入
                    x1 = y[m.f] if isinstance(m.f, int) else [x1 if j == -1 else y[j] for j in m.f]  # from earlier layers
                x1 = m(x1)
                ...
                if profile:
                    o = thop.profile(m, inputs=(x2,), verbose=False)[0] / 1E9 * 2 if thop else 0  # FLOPS
                    t = time_synchronized()
                    for _ in range(10):
                        _ = m(x2)
...
def parse_model(d, ch, ch2):  # model_dict, input_channels(3)
    ...
    anchors, nc, gd, gw, n_segcls, n_cls, depth_1 = d['anchors'], d['nc'], d['depth_multiple'], d['width_multiple'], d['n_segcls'], d['n_cls'], d['depth_1']
    ...
    for i, (f, n, m, args) in enumerate(d['backbone1'] + d['backbone2'] + d['head']):  # from, number, module, args
        ...
        if i < depth_1:              #backbone1, rgb图像输入经过，ch[]记录输出
            if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv,
                     BottleneckCSP, C3, C3TR, ASPP, SPPF, C2f]:
        ...
        else:                        #backbone2,depth图像输入经过，head,rgb 和 depth concat之后输入，ch2[]记录输出
            if m in [Conv, Focus, CrossConv, C3, C3TR, SPPF, C2f, C3_SA, C2f_SAConv, C2f_DLKA, C3_DLKA, P_DLK]:
                c12, c22 = ch2[f], args[0]  # 指定层输入(c1)输出(c2)通道数(ch记录各层输出通道,f表输入层下标,输入层的输出通道就是本层输入通道)
                if c22 != no:  # if not output 对非输出层, 原作者此处代码有风险
                    c22 = make_divisible(c22 * gw, 8)  # 实际输出通道数是 配置文件的c2 * width_multiple 并向上取到可被8整除

                args = [c12, c22, *args[1:]]
                if m in [C3, C3_SA, C2f_SAConv, C2f_DLKA, C3_DLKA]:
                    args.insert(2, n)  # number of repeats 对C3和BottleneckCSP来说深度n代表残差模块的个数, C3TR的n表transformer的head数
                    n = 1  # 置1表示深度对这三个模块是控制子结构重复, 而不是本身重复
            elif m is nn.BatchNorm2d:
                args = [ch2[f]]  # 对BN层, 参数就是输入层的通道数
            elif m is Concat:
                c22 = sum([ch2[x] for x in f])  # Concat层, 输出通道就是几个输入层通道数相加
            elif m in [SEFusion, FRM, CAFusion, PointwiseConv, CAFuse]:
                c22 = args[0]  # Fusion层, 输出通道就是输入层通道数
                c22 = make_divisible(c22 * gw, 8)
                args = [c22]
            elif m is Detect:
                args.append([ch2[x] for x in f])  # 检测层, 把来源下标列表f中的层输出通道数加入args中, 用于构建Detect的卷积输入通道数
        ...
        if i == depth_1:
            ch2 = ch
        ch2.append(c22)  # 保存此层输出通道数, 下一层输入通道
    return nn.Sequential(*layers), sorted(save), depth_1


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--cfg', type=str, default='mtm_de.yaml', help='model.yaml')
