...
class BaseDataset(data.Dataset):
    ...
    def _testval_img_transform(self, img, img2):  # 新的训练后测验证集数据处理(仅支持同尺寸图): 图长边resize到base_size, 但标签是原图, 若非原图需要测试时手动把输出放大到原图 (原版仅处理标签, 原图输入)
        w, h = img.size
    ...
    def _val_sync_transform(self, img, img2, mask):  # 训练中验证数据处理(支持不同尺寸图，但是指标通常比testval略低一点点): 把图短边resize成crop_size, 长边保持比例, 再crop一块(crop_size,crop_size)用于验证(在citysbdd和custom中图不同时候使用)
        ...
        img2 = img2.crop((x1, y1, x1 + outsize, y1 + outsize))
        mask = mask.crop((x1, y1, x1+outsize, y1+outsize))
        # final transform
        # return img, self._mask_transform(mask)
        return img, img2, mask  # 这里改了, 在__getitem__里再调用self._mask_transform(mask)

    def _sync_transform(self, img, img2, mask):  # 训练数据增广
        # random mirror
        # m = random.random()
        # if m < 0.5:
        ...
        img2 = img2.crop((x1, y1, x1+w_crop_size, y1+h_crop_size))
...
class CitySegmentation(BaseDataset):  # base_size 2048 crop_size 768
    ...
    def __init__(self, root=os.path.expanduser('../data/citys/'), split='train',
        ...
        self.images, self.images2, self.mask_paths = get_city_pairs(self.root, self.split)
    ...
    def _class_to_index(self, mask):
        ...
        # output = self._key[index].reshape(mask.shape)
        # return self._key[index].reshape(mask.shape)
        return mask

    def __getitem__(self, index):
        img = Image.open(self.images[index]).convert('RGB')
        img2 = Image.open(self.images2[index]).convert('RGB')
        if self.mode == 'test':
            if self.transform is not None:
                img = self.transform(img)
                img2 = self.transform(img2)
            return img, os.path.basename(self.images[index]), img2, os.path.basename(self.images2[index])
        # mask = self.masks[index]
        mask = Image.open(self.mask_paths[index])
        # synchrosized transform
        if self.mode == 'train':
            img, img2, mask = self._sync_transform(img, img2, mask)  # 训练数据增广
            mask = self._mask_transform(mask)
        elif self.mode == 'val':
            img, img2, mask = self._val_sync_transform(img, img2, mask)  # 验证数据处理
            mask = self._mask_transform(mask)
        else:
            ...
            img, img2 = self._testval_img_transform(img, img2)
            mask = self._mask_transform(mask)
        ...
        return img, img2, mask, self.images[index], self.images2[index]
...
def get_city_pairs(folder, split='train'):
    def get_path_pairs(img_folder, mask_folder):
        ...
        for root, directories, files in os.walk(img_folder):
            for filename in files:
                ...
                if filename.endswith(".png") or filename.endswith(".jpg"):
                    imgpath = os.path.join(root, filename)
                    imgpath2 = imgpath.replace('images', 'images2').replace('color', 'RGB_depth')
                    maskname = filename.replace('.jpg', '.png')
                    maskpath = os.path.join(mask_folder, maskname)
                    if os.path.isfile(imgpath) and os.path.isfile(maskpath) and os.path.isfile(imgpath2):
                        img_paths.append(imgpath)
                        img_paths2.append(imgpath2)
                        mask_paths.append(maskpath)
                    else:  # 正常情况Cityscapes和BDD数据文件层面很干净不应该警告
                        print('cannot find the mask or image:', imgpath, imgpath2, maskpath)
        print('Found {} images in the folder {}'.format(len(img_paths), img_folder))
        return img_paths, img_paths2, mask_paths
    ...
    if split == 'train' or split == 'val' or split == 'test':
        ...
        img_paths, img_paths2, mask_paths = get_path_pairs(img_folder, mask_folder)
        return img_paths, img_paths2, mask_paths
    else:
        ...
        train_mask_folder = os.path.join(folder, 'labels/train')
        val_img_folder = os.path.join(folder, 'images/val')
        val_mask_folder = os.path.join(folder, 'labels/val')
        train_img_paths, train_img_paths2, train_mask_paths = get_path_pairs(train_img_folder, train_mask_folder)
        val_img_paths, val_img_paths2, val_mask_paths = get_path_pairs(val_img_folder, val_mask_folder)
        img_paths = train_img_paths + val_img_paths
        img_paths2 = train_img_paths2 + val_img_paths2
...
def get_citys_loader(root=os.path.expanduser('data/citys/'), split="train", mode="train",  # 获取训练和验证用的dataloader
    ...
    # torch.manual_seed(6148914696321517205 + RANK)
