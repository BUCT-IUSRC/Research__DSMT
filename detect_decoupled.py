...
from utils.datasets import LoadStreams, LoadImages, cls_names, Load_depth_Images
from utils.general import check_img_size, check_requirements, check_imshow, non_max_suppression, apply_classifier, \
    scale_coords,scale_boxes, xyxy2xywh, strip_optimizer, set_logging, increment_path
from utils.plots import plot_one_box
from utils.torch_utils import select_device, load_classifier, time_synchronized
import numpy as np
import classify.predict as cls_pred
...
def detect(save_img=False):
    ...
    if webcam:
        ...
        dataset = Load_depth_Images(source, source2, img_size=imgsz, stride=stride)  #多模态数据加载
    ...
    for path, path2, img, im0s, img2, im0s2, vid_cap, s in dataset:
        ...
        with torch.no_grad():
            ...
            out = model(img, img2, augment=opt.augment)
            pred = out[0]
            seg = out[1]  # [0]
        # Apply NMS
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    annotator = Annotator(dst, example=str(cls_names), pil=True)
                    # print(i, l[i])
                    annotator.text((32, 32), l[sn], txt_color=(255, 255, 255))
                    im1 = annotator.result()
                    cv2.imwrite(save_path[:-4]+"_cls"+save_path[-4:], im1)
                    sn += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source2', type=str, default='data/images2', help='depth_source')  # file/folder, 0 for webcam
