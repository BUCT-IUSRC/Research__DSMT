...
import test_decoupled  # import test.py to get mAP after each epoch
from models.experimental import attempt_load
from models.yolo import Model
from utils.autoanchor import check_anchors
from utils.datasets import create_dataloader, create_classification_dataloader
from utils.general import labels_to_class_weights, increment_path, labels_to_image_weights, init_seeds, \
    fitness, fitness2, fitness3, strip_optimizer, get_latest_run, check_dataset, check_file, check_git_status, check_img_size, \
    check_requirements, print_mutation, set_logging, one_cycle, colorstr
from utils.google_utils import attempt_download
from utils.loss import ComputeLoss, SegmentationLosses
from utils.plots import plot_images, plot_labels, plot_results, plot_evolution
from utils.torch_utils import ModelEMA, select_device, intersect_dicts, torch_distributed_zero_first, is_parallel, smartCrossEntropyLoss
...
def train(hyp, opt, device, tb_writer=None):
    ...
    save_dir, epochs, batch_size, total_batch_size, weights, rank, bs = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.total_batch_size, opt.weights, opt.global_rank, opt.cls_bs
    ...
    if pretrained:  # 有预训练参数
        ...
        model = Model(opt.cfg or ckpt['model'].yaml, ch=3, ch2=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
        ...
        model = Model(opt.cfg, ch=3, ch2=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
    with torch_distributed_zero_first(rank):
        check_dataset(data_dict)  # check
    train_path = data_dict['train']
    train_path2 = data_dict['train2']
    test_path = data_dict['val']
    test_path2 = data_dict['val2']                 #检测深度图像地址
    ...
    data_dir = Path('./data/cls')
    cls_nc = len([x for x in (data_dir / 'train').glob('*') if x.is_dir()])  # number of classes
    test_dir = data_dir / 'test' if (data_dir / 'test').exists() else data_dir / 'val'
    WORLD_SIZE = int(os.getenv('WORLD_SIZE', 1))
    LOCAL_RANK = int(os.getenv('LOCAL_RANK', -1))
    nw1 = min(os.cpu_count() - 1, opt.workers)
    # bs = 18             # 分类batchsize
    ...
    # 检测 Trainloader
    dataloader, dataset = create_dataloader(train_path, train_path2, imgsz, batch_size, gs, opt,
                                            hyp=hyp, augment=True, cache=opt.cache_images, rect=opt.rect, rank=rank,
                                            world_size=opt.world_size, workers=opt.workers,
                                            image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('train: '), prefix2=colorstr('train2: '))
    ...
    if rank in [-1, 0]:
        testloader = create_dataloader(test_path, test_path2, imgsz_test, batch_size * 2, gs, opt,  # testloader batch_size翻倍
                                       hyp=hyp, cache=opt.cache_images and not opt.notest, rect=True, rank=-1,
                                       world_size=opt.world_size, workers=opt.workers,
                                       pad=0.5, prefix=colorstr('val: '), prefix2=colorstr('val2: '))[0]  # [0]只要了loader没要dataset, 和train处理不一样
    ...
    # 分割 loader   citys和bdd的basesize都取1024,crop_size长边取imgsz,短边imgsz砍半
        seg_valloader = SegmentationDataset.get_citys_loader(root=segval_path, batch_size=4,
                                                         ...
                                                         base_size=640,   # 对cityscapes, 原图resize到(1024, 512)输入后双线性插值到原图尺寸计算精度
    ...
    # 分类loder
        cls_testloader = create_classification_dataloader(path=test_dir,
                                                      imgsz=224,
    ...
    cls_trainloader = create_classification_dataloader(path=data_dir / 'train',
                                                   imgsz=224,
    ...
    seg_trainloader = SegmentationDataset.get_citys_loader(root=segtrain_path,
                                                           ...
                                                           base_size=640, crop_size=(imgsz, imgsz//2),  # cropsize长边和检测同，短边砍半(针对cityscapes,bdd也行)
...
#-----------------------------------------------------------------------------------------------------------
    ...
    detgain, seggain, clsgain = 0.65, 0.3, 0.05  # 检测, 分割比例
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        mloss = torch.zeros(3, device=device)  # 检测 mean losses
        msegloss = torch.zeros(1, device=device)  # 混合的 mean losses, 两者计算也可知分割loss
        mclsloss = torch.zeros(1, device=device)
        if rank != -1:
            dataloader.sampler.set_epoch(epoch)  # shuffle时, 保证每个epoch顺序不同
        pbar = enumerate(dataloader)
        segpbar = enumerate(seg_trainloader)
        clspbar = enumerate(cls_trainloader)
        logger.info(('\n' + '%10s' * 9) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'total', 'seg', 'labels', 'img_size'))
        if rank in [-1, 0]:
            pbar = tqdm(pbar, total=min(nb, segnb, len(cls_trainloader)))  # progress bar # tqdm进度条迭代
            segpbar = tqdm(segpbar, total=min(nb, segnb, len(cls_trainloader)))
            clspbar = tqdm(clspbar, total=min(nb, segnb, len(cls_trainloader)), bar_format=TQDM_BAR_FORMAT)
        optimizer.zero_grad()  # 每轮前清空梯度

        # 暂时用zip, 每轮batch数以数量少的为准
        for det_batch, seg_batch, cls_batch in zip(pbar, segpbar, clspbar):  # batch -------------------------------------------------------------
            i, (imgs, imgs2, targets, paths, path2, _, _) = det_batch  # 检测
            _, (segimgs, segimgs2, segtargets, seg_path, seg_path2) = seg_batch   # 分割
            _, (clsimgs, clsimgs2, clstargets, cls_path, cls_path2) = cls_batch   # 分类
            if len(imgs)==1 or len(segimgs)==1 or len(clsimgs) == 1:  # 手动droplast,SE或者gloablpool后的bn不支持单个样本，检测loader调用地方太多不好droplast，这里手动
                continue
            # warmup等参数变化以检测为准
            ni = i + nb * epoch  # number integrated batches (since train start) 记录总iterations, 可以用于停止warmup
            imgs = imgs.to(device, non_blocking=True).float() / 255.0  # uint8 to float32, 0-255 to 0.0-1.0
            imgs2 = imgs2.to(device, non_blocking=True).float() / 255.0
            # Warmup
            if ni <= nw:
                xi = [0, nw]  # x interp
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ...
                    imgs2 = F.interpolate(imgs2, size=ns, mode='bilinear', align_corners=False)
            ...
            if plots and ni >= 3:
                ...
                imgs2 = imgs2.to(torch.device('cpu'), non_blocking=True)
            
            segimgs = segimgs.to(device, non_blocking=True)  # 分割已经做过totensor了, 不用/255
            segimgs2 = segimgs2.to(device, non_blocking=True)

            with amp.autocast(enabled=cuda):  # 混合精度训练中用来代替autograd
                pred = model(segimgs, segimgs2)
#-----------------------------------------------------------------------------------------------------------
...
#-----------------------------------------------------------------------------------------------------------                         
            ...
            del segimgs, segimgs2

            #分类训练
            # clsimgs, clstargets = clsimgs.to(device, non_blocking=True), clstargets.to(device)
            # clsimgs2 = clsimgs2.to(device, non_blocking=True)
            # with amp.autocast(enabled=cuda):  # stability issues when enabled
            #     pred = model(clsimgs, clsimgs2)
            #     clsloss = criterion(pred[2], clstargets)
            #     clsloss *= clsgain
            #
            # scaler.scale(clsloss).backward()
            # del clsimgs, clsimgs2
            ...
            # Optimize
            if ni % accumulate == 0:  # 梯度积累accumulate次后才优化, ni: iter
            ...
            if rank in [-1, 0]:
                ...
                # mclsloss = (mclsloss * i + clsloss.item()) / (i + 1)
                mclsloss = 0
                mem = '%.3gG' % (torch.cuda.memory_reserved() / 1E9 if torch.cuda.is_available() else 0)  # (GB)
                s = ('%10s' * 2 + '%10.4g' * 7) % (
                    '%g/%g' % (epoch, epochs - 1), mem, *mloss, msegloss, mclsloss, targets.shape[0], imgshape)
                ...
                if plots and ni < 3:
                    ...
                    #深度图像train_batch显示
                    f1 = save_dir / f'train_batch2_{ni}.jpg'  # filename
                    Thread(target=plot_images, args=(imgs2, targets, path2, f1), daemon=True).start()
                    # if tb_writer:
        ...
        if rank in [-1, 0]:
            ...
            if epoch % 10 == 0 or (epochs - epoch) < 40:
                mIoU = test_decoupled.seg_validation(model=ema.ema, valloader=seg_valloader, device=device, n_segcls=5,
                                half_precision=True)
                top1, top5, vloss = validate.run(model=ema.ema,
                                                 dataloader=cls_testloader,
                                                 criterion=criterion,
                                                 pbar=clspbar)  # test accuracy, loss
                accuracy = top1  # define fitness as top1 accuracy
                logger.info(f'accuracy: {top1} \n'
                            f'vloss {vloss} \n')
            # mAP
            final_epoch = epoch + 1 == epochs  # 是否是最后一轮
            if not opt.notest or final_epoch:  # Calculate mAP
                wandb_logger.current_epoch = epoch + 1
                results, maps, times = test_decoupled.test(data_dict,
            ...
            fi = fitness3(np.array(results).reshape(1, -1), mIoU, accuracy)  # weighted combination of [P, R, mAP@.5, mAP@.5-.95] 按0.1*AP.5+0.9*AP.5:.95指标衡量模型
            if fi > best_fitness:
                best_fitness = fi
    ...
    if rank in [-1, 0]:
        ...
        if opt.data.endswith('coco.yaml') and nc == 80:  # if COCO
            for m in (last, best) if best.exists() else (last):  # speed, mAP tests
                results, _, _ = test_decoupled.test(opt.data,
                                          batch_size=batch_size * 2,
                                          imgsz=imgsz_test,
                                          conf_thres=0.001,
                                          iou_thres=0.7,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='best_271.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default='mtm_de.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='data/cityscapes_det.yaml', help='data.yaml path')
    parser.add_argument('--hyp', type=str, default='data/hyp.scratch.yaml', help='hyperparameters path')
    parser.add_argument('--epochs', type=int, default=100)
    parser.add_argument('--batch-size', type=int, default=18, help='total batch size for all GPUs')
    parser.add_argument('--cls-bs', type=int, default=18, help='cls batch size for all GPUs')
    parser.add_argument('--img-size', nargs='+', type=int, default=[832, 832], help='[train, test] image sizes')
